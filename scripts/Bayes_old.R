...
#
# library(readr)
# Signordb_human <- read_delim("C:/Users/Marie/Desktop/Master_Thesis/Signordb_human.tsv",
...
get_node_expression_vals <- function(edgelist){
  ...
  for (x in names(layers)){

    if(!is.null(layers[[x]])){
      data_entry <- paste0(x, "_counts")
      #print(data_entry)
      cluster_exp <- rbind(cluster_exp, data[[data_entry]][rownames(data[[data_entry]]) %in% genes,]
                           # %>%
                           #   tibble::add_column(., entity = base::strsplit(data_entry, split = "_") %>%
                           #                        unlist(.) %>%
                           #                        magrittr::extract2(1))
                           )
    }
